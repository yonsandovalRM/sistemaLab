...
Module Modulo_Imprime_Resultado_Tejidos
    ...
    Public Sub Genera_Impresion_Tejidos(ByVal ChkTransSecFre As Boolean, ByVal OTResultado As Integer, ByVal NLabD As Integer, ByVal NLabH As Integer)
        ...
        If Retorno = 1 Then
            sRutaDest = "F:\Mis Documentos\" + sAño + ".Frutos-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & ".xls"
        Else
            sRutaDest = "F:\Mis Documentos\" + sAño + ".Frutos-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & " NumOrden " & OrdenEnResulatdos & ".xls"
        End If


        'sRutaDest = "F:\Mis Documentos\LabSys-Pruebas\" + sAño + ".Foliar-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & ".xls"
        ...
        For Each Fila_Ciclo In Cjto_Tablas1.Tables("TABLA_CICLO").Rows
            ...
            If SALTO_PAGINA = True Or Cuenta_Columnas > Maximo_columnas Then
                Call Coloca_Columnas_Tejidos(Cuenta_Columnas)
                Call Imprime_Encabezado(Hoja_Excel, pagina, Fila, Fila_Firma, OrdenEnResulatdos)
                Fila_Ini = Fila
                Call Imprime_Detalle(ChkTransSecFre, Hoja_Excel, Fila, m_Excel)
                Fila_Fin = Fila - 1
...
SaltoxCicloInicial:
        ...
        Call Coloca_Columnas_Tejidos(Cuenta_Columnas + 1)
        '*************************
        SALTO_PAGINA = True
        Call Revisa_Salto_Por_Encabezado(Fila_Ciclo, SALTO_PAGINA, Cuenta_Columnas)
        Call Imprime_Encabezado(Hoja_Excel, pagina, Fila, Fila_Firma, OrdenEnResulatdos)
    ...
    Private Sub Coloca_Columnas_Tejidos(ByVal cuenord As Integer)
    ...
    Private Sub Imprime_Encabezado(ByVal HOJA_EXCEL As Excel.Worksheet, ByVal pagina As Integer, ByRef fila As Integer, ByRef Fila_Firma As Integer, ByVal OTResultado As Integer)
        ...
        HOJA_EXCEL.Range("AJ" & fila.ToString).Value = "ANALISIS TEJIDOS"
    ...
    Private Sub Imprime_Detalle(ByVal ChkTransSecFre As Boolean, ByVal HOJA_EXCEL As Excel.Worksheet, ByRef Fila As Integer, ByVal ExcelA As Excel.Application)
        ...
        For Each Fila_Elemento In Cjto_Tablas1.Tables("ELEMENTO_PRIORIDAD").Rows
            ...
            For Each Fila_Nlab In Cjto_Tablas1.Tables("NLAB_COLUMNAS").Rows
                ...
                If (Fila_DigRes Is Nothing) = False Then
                    ...
                    If Fila_Elemento("ELE_CODIGO") <> "MSeca_TejHumedo" Then
                    ...
                    If IsDBNull(Fila_DigRes("DIGRES_RESULTADO")) = False Then
                        ...
                        If ChkTransSecFre = True Then Call TransformaMatSeca(Fila_Elemento("ELE_CODIGO"), Resultado, Fila_Nlab("OT_NLAB"))
                        HOJA_EXCEL.Range(COLD & Fila.ToString & ":" & COLH & Fila.ToString).Merge()
    ...
    Private Sub TransformaMatSeca(ByVal CodElem As String, ByRef ResulParc As Double, ByVal OtNLab As Integer)
        Dim CampoMatSeca As Double
        Dim ds As DataSet = New DataSet
        Dim SP As Procedimiento_Almacenado = New Procedimiento_Almacenado
        ...
        With SP
            .Inicializar()
            .AgregarParametro("@Tmuestra", 2000, SqlDbType.Int)
            .AgregarParametro("@CodEle", "MSeca_TejHumedo", SqlDbType.NVarChar)
            .AgregarParametro("@NLab", OtNLab, SqlDbType.Int)
            ds = .EjecutarQuery("usp_SelectMateriaSeca")
        End With

        If IsDBNull(ds.Tables(0).Rows(0).Item("DIGRES_RESULTADO")) = False Then
            CampoMatSeca = ds.Tables(0).Rows(0).Item("DIGRES_RESULTADO")
            If CodElem = "Nt_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10) / 10000), "##.##")
            If CodElem = "NNO3_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "NNH4_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "P_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10) / 10000), "##.##")
            If CodElem = "K_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10) / 10000), "##.##")
            If CodElem = "Ca_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10) / 10000), "##.##")
            If CodElem = "Mg_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10) / 10000), "##.##")
            If CodElem = "Zn_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "Mn_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "Fe_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "Cu_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
            If CodElem = "B_TejHumedo" Then ResulParc = Format(((ResulParc * (100 / CampoMatSeca) * 10)), "##.##")
        End If
    ...
    Public Sub Notas_Automaticas(ByVal esp As String, ByVal HOJA_EXCEL As Excel.Worksheet, ByVal Fila_Fin As Integer)
        ...
        If Not (FILA_NOTA Is Nothing) Then
            If IsDBNull(FILA_NOTA("DESCRIPCION")) <> True Then
                ...
                HOJA_EXCEL.Range("A" & (Fila_Fin).ToString).Value = "Análisis realizado en fruto fresco y expresado en mg/100g f.fresco"
