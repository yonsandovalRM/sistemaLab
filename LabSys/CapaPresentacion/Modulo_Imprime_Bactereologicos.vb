...
Module Modulo_Imprime_Bactereologicos
    ...
    Public Sub Genera_Impresion_Bactereologicos(ByVal OTResultado As Integer, ByVal NLabD As Integer, ByVal NLabH As Integer)
        ...
        End With
        If Retorno = 1 Then
            sRutaDest = "F:\Mis Documentos\" + sAño + ".Bactereologicos-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & ".xls"
        Else
            sRutaDest = "F:\Mis Documentos\" + sAño + ".Bactereologicos-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & " NumOrden " & OrdenEnResulatdos & ".xls"
        End If
        'sRutaDest = "F:\Mis Documentos\LabSys-Pruebas\" + sAño + ".Bactereologicos-Labsys" : destino = sRutaDest & "\" & NLAD & NLAH & "   " & PRO & ".xls"
...
SaltoxCicloInicial:
    ...
    Private Sub Imprime_Encabezado(ByVal HOJA_EXCEL As Excel.Worksheet, ByVal pagina As Integer, ByRef fila As Integer, ByRef Fila_Firma As Integer, ByVal OTResultado As Integer)
        ...
        '    HOJA_EXCEL.Range("CJ" & fila.ToString).Value = "Interpretación"
        ...
        '    HOJA_EXCEL.Range("CJ" & (fila + 2).ToString).Value = "Limite máximo"
        ...
        '    HOJA_EXCEL.Range("CV" & (fila).ToString).Value = "Método"
    ...
    Public Sub Notas_Automaticas(ByVal OTResultado As String, ByVal HOJA_EXCEL As Excel.Worksheet, ByVal Fila_Fin As Integer)
        ...
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString).Value = "Parámetros en que la NCh409 no señala límites máximos no se encuentran regulados en la norma."
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString).Font.Size = 8
        ...
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString & ":DF" & (Fila_Fin).ToString).Merge()
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString).Value = "- Analisis realizados de acuerdo a la norma NCh17025 en laboratorio Cesmec Ltda., laboratorio con acreditación INN LE 076 y LE 189."
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString).Font.Size = 8
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString & ":DF" & (Fila_Fin).ToString).HorizontalAlignment = Excel.XlHAlign.xlHAlignJustify
        HOJA_EXCEL.Range("A" & Fila_Fin.ToString & ":DF" & (Fila_Fin).ToString).VerticalAlignment = Excel.XlVAlign.xlVAlignTop
