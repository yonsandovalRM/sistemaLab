...
Public Class DbManager
    ...
    Public Sub Update(ByVal datos As DataSet)
        Dim updatequery, temp, ValGra As String
        Dim i, j, flag, Mypos As Integer
        Dim fila As DataRow
        ...
        For Each fila In datos.Tables(Me.tabla).Rows
            If fila.RowState <> DataRowState.Deleted Then
                ...
                For i = 0 To 100
                    Try
                        If IsNumeric(fila.Item(i)) Then
                            Mypos = InStr(CStr(fila.Item(i)), ",")
                            If Mypos > 0 Then
                                ValGra = Mid(CStr(fila.Item(i)), 1, Mypos - 1) & "." & Mid(CStr(fila.Item(i)), Mypos + 1)
                            Else
                                ValGra = CStr(fila.Item(i))
                            End If
                            updatequery = updatequery & datos.Tables(Me.tabla).Columns(i).ColumnName & "=" & ValGra & ","
                            'updatequery = updatequery & datos.Tables(Me.tabla).Columns(i).ColumnName & "=" & CStr(fila.Item(i)) & ","
                        Else
    ...
    Public Sub Selecccionar(ByRef datos As DataSet, ByVal query As String)
        Try
            conexion = New SqlConnection(Conexion1)
